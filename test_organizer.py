...
def test_environment(safe_tmp_path):
    ...
    # sparse file — occupies no real disk space but reports 2MB size
    with open(source_dir / "medium_file.bin", 'wb') as _f:
...
class TestOrganizeBySize:
    ...
    def test_large_file(self, safe_tmp_path):
        ...
        # sparse file — uses truncate so no real data is written
        with open(large_file, 'wb') as f:
            f.truncate(101 * 1024 * 1024)  # 101MB sparse
...
# ═══════════════════════════════════════════════════════════════
#                    AI CONTENT ORGANIZER TESTS
# ═══════════════════════════════════════════════════════════════

class TestAIContentOrganizer:
    """Unit tests for AI content organizer components."""

    def test_is_project_folder_with_package_json(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        (tmp_path / "package.json").write_text("{}")
        analyzer = ContentAnalyzer(None, None)
        assert analyzer.is_project_folder(tmp_path) is True

    def test_is_project_folder_with_requirements_txt(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        (tmp_path / "requirements.txt").write_text("flask\n")
        analyzer = ContentAnalyzer(None, None)
        assert analyzer.is_project_folder(tmp_path) is True

    def test_is_project_folder_with_git(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        (tmp_path / ".git").mkdir()
        analyzer = ContentAnalyzer(None, None)
        assert analyzer.is_project_folder(tmp_path) is True

    def test_is_project_folder_with_makefile(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        (tmp_path / "Makefile").write_text("build:\n\tgcc main.c\n")
        analyzer = ContentAnalyzer(None, None)
        assert analyzer.is_project_folder(tmp_path) is True

    def test_is_project_folder_empty(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        analyzer = ContentAnalyzer(None, None)
        assert analyzer.is_project_folder(tmp_path) is False

    def test_is_project_folder_only_random_files(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
    ...
    def test_get_folder_structure_max30(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        for i in range(40):
            (tmp_path / f"file{i}.txt").write_text("")
        analyzer = ContentAnalyzer(None, None)
        structure = analyzer.get_folder_structure(tmp_path)
        assert len(structure.split("\n")) <= 30

    def test_get_folder_structure_contains_filenames(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
    ...
    def test_read_file_preview_truncates(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer, CONTENT_PREVIEW_CHARS
        f = tmp_path / "test.txt"
        f.write_text("A" * 3000)
        analyzer = ContentAnalyzer(None, None)
        preview = analyzer.read_file_preview(f)
        assert preview is not None
        assert len(preview) <= CONTENT_PREVIEW_CHARS

    def test_read_file_preview_returns_content(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        f = tmp_path / "invoice.txt"
        f.write_text("Invoice #12345\nAmount: $500")
        analyzer = ContentAnalyzer(None, None)
        preview = analyzer.read_file_preview(f)
        assert preview is not None
        assert "Invoice" in preview

    def test_sanitize_folder_name_removes_slashes(self):
        from ai_content_organizer import ContentAnalyzer
        result = ContentAnalyzer._sanitize_folder_name("Finance/Reports")
        assert "/" not in result

    def test_sanitize_folder_name_removes_backslash(self):
        from ai_content_organizer import ContentAnalyzer
        result = ContentAnalyzer._sanitize_folder_name("C:\\docs")
        assert "\\" not in result

    def test_sanitize_folder_name_removes_special_chars(self):
        from ai_content_organizer import ContentAnalyzer
        for char in [':', '*', '?', '"', '<', '>', '|']:
            result = ContentAnalyzer._sanitize_folder_name(f"folder{char}name")
            assert char not in result

    def test_sanitize_folder_name_empty_returns_uncategorized(self):
        from ai_content_organizer import ContentAnalyzer
        result = ContentAnalyzer._sanitize_folder_name("///\\\\")
        assert result == "Uncategorized"

    def test_sanitize_folder_name_valid_unchanged(self):
        from ai_content_organizer import ContentAnalyzer
        result = ContentAnalyzer._sanitize_folder_name("Python Scripts")
        assert result == "Python Scripts"

    def test_ollama_client_fetch_models_returns_empty_on_failure(self):
        from ai_content_organizer import OllamaClient
        client = OllamaClient("http://localhost:1")
        models = client.fetch_models()
        assert isinstance(models, list)
        assert models == []

    def test_ollama_client_is_connected_false_when_down(self):
        from ai_content_organizer import OllamaClient
        client = OllamaClient("http://localhost:1")
        assert client.is_connected() is False

    def test_ollama_client_get_all_models_includes_cloud(self):
        from ai_content_organizer import OllamaClient, CLOUD_MODELS
        client = OllamaClient("http://localhost:1")
        models = client.get_all_models()
        for cloud_model in CLOUD_MODELS:
            assert cloud_model in models

    def test_ollama_client_get_all_models_no_duplicates(self):
        from ai_content_organizer import OllamaClient
        client = OllamaClient("http://localhost:1")
        models = client.get_all_models()
        assert len(models) == len(set(models))

    def test_organize_by_content_fallback(self, safe_tmp_path):
        source = safe_tmp_path / "source"
        dest = safe_tmp_path / "dest"
        source.mkdir()
        test_file = source / "test.txt"
        test_file.write_text("hello")
        result = file_organizer.organize_by_content(
            test_file, dest,
            action="move", conflict_policy="rename", dry_run=False
        )
        assert result is True
        assert (dest / "AI_Organized" / "test.txt").exists()

    def test_organize_by_content_in_organizers_dict(self):
        assert "content" in file_organizer.ORGANIZERS

    def test_text_extensions_set_not_empty(self):
        from ai_content_organizer import TEXT_EXTENSIONS
        assert ".txt" in TEXT_EXTENSIONS
        assert ".py" in TEXT_EXTENSIONS
        assert ".md" in TEXT_EXTENSIONS

    def test_project_indicators_set_not_empty(self):
        from ai_content_organizer import PROJECT_INDICATORS
        assert "package.json" in PROJECT_INDICATORS
        assert "requirements.txt" in PROJECT_INDICATORS
        assert ".git" in PROJECT_INDICATORS

    def test_cloud_models_defined(self):
        from ai_content_organizer import CLOUD_MODELS
        assert isinstance(CLOUD_MODELS, list)
        assert len(CLOUD_MODELS) >= 1

    def test_read_file_preview_utf8(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        f = tmp_path / "arabic.txt"
        f.write_text("مرحبا بالعالم\nHello World", encoding="utf-8")
        analyzer = ContentAnalyzer(None, None)
        preview = analyzer.read_file_preview(f)
        assert preview is not None
        assert "مرحبا" in preview

    def test_get_folder_structure_empty_folder(self, tmp_path):
        from ai_content_organizer import ContentAnalyzer
        analyzer = ContentAnalyzer(None, None)
        structure = analyzer.get_folder_structure(tmp_path)
