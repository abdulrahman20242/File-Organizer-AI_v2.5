...
def organize_by_content(file: Path, dest_root: Path, **kwargs) -> Optional[bool]:
    """
    Wrapper placeholder — actual execution happens in AIContentWorker.
    Used only if process_directory is called with mode='content' directly.
    """
    dest_file = dest_root / "AI_Organized" / file.name
    final_dest = resolve_conflict(dest_file, kwargs['conflict_policy'])
    if final_dest is None:
        return None
    return do_transfer(file, final_dest, kwargs['action'], kwargs['dry_run'])
