...
from ai_content_organizer import (
    AIContentWorker, AIPreviewDialog, OllamaClient, ModelRefreshWorker
)
...
class FileOrganizerGUI(QMainWindow):
    ...
    def _create_main_layout(self):
        ...
        self.ai_settings_group = QGroupBox("AI Settings")
        self.ai_settings_group.setVisible(False)
        ai_layout = QFormLayout(self.ai_settings_group)
        
        self.txt_ollama_url = QLineEdit("http://localhost:11434")
        ai_layout.addRow("Ollama URL:", self.txt_ollama_url)
        
        model_row = QHBoxLayout()
        self.cmb_ai_model = QComboBox()
        self.btn_refresh_models = QPushButton(qta.icon('fa5s.sync'), "Refresh")
        self.lbl_ollama_status = QLabel("● Disconnected")
        self.lbl_ollama_status.setStyleSheet("color: red;")
        model_row.addWidget(self.cmb_ai_model)
        model_row.addWidget(self.btn_refresh_models)
        model_row.addWidget(self.lbl_ollama_status)
        ai_layout.addRow("Model:", model_row)
        
        main_layout.addWidget(self.ai_settings_group)
    ...
    def _get_current_settings_dict(self):
        return {
            ...
            "ai_ollama_url": self.txt_ollama_url.text(),
            "ai_model": self.cmb_ai_model.currentText(),
        }
    ...
    def _apply_settings_dict(self, s: dict):
        ...
        if hasattr(self, 'txt_ollama_url'):
            self.txt_ollama_url.setText(s.get("ai_ollama_url", "http://localhost:11434"))
        if hasattr(self, 'cmb_ai_model'):
            saved = s.get("ai_model", "")
            if saved:
                idx = self.cmb_ai_model.findText(saved)
                if idx >= 0:
    ...
    def run_organizer(self):
        ...
        # Handle AI content mode separately
        if mode == "content":
            ...
            if not client.is_connected():
                ...
                return
            self._run_ai_organizer(source, dest)
            return
    ...
    def _on_mode_changed(self):
        """Show/hide AI settings panel based on selected mode."""
        is_ai_mode = self.cmb_mode.currentData() == "content"
        self.ai_settings_group.setVisible(is_ai_mode)
        if is_ai_mode:
    ...
    def _refresh_ollama_models(self):
        ...
        self.lbl_ollama_status.setText("● Connecting...")
        self.lbl_ollama_status.setStyleSheet("color: orange;")

        self._model_refresh_worker = ModelRefreshWorker(url)
        self._model_refresh_worker.models_ready.connect(self._on_models_ready)
        self._model_refresh_worker.error.connect(self._on_models_error)
        self._model_refresh_worker.start()

    def _on_models_ready(self, models: list):
        self.btn_refresh_models.setEnabled(True)
        self.lbl_ollama_status.setText("● Connected")
        self.lbl_ollama_status.setStyleSheet("color: green;")
        
        saved_model = self.cmb_ai_model.currentText()
        self.cmb_ai_model.clear()
        self.cmb_ai_model.addItems(models)
        
        # Restore previously selected model
        if saved_model:
            idx = self.cmb_ai_model.findText(saved_model)
            if idx >= 0:
                self.cmb_ai_model.setCurrentIndex(idx)

    def _on_models_error(self, error_msg: str):
        self.btn_refresh_models.setEnabled(True)
        self.lbl_ollama_status.setText("● Disconnected")
        self.lbl_ollama_status.setStyleSheet("color: red;")
        self.log_signal.emit(f"⚠️ Could not fetch models: {error_msg}")

    def _run_ai_organizer(self, source: Path, dest: Path):
        """Starts the AI analysis workflow."""
        self.set_controls_enabled(False)
        self.lbl_status.setText("Analyzing content with AI...")
        self.progress.setVisible(True)
        ...
        self.ai_worker = AIContentWorker(
            source=source,
            dest=dest,
        ...
        self.ai_worker.log_message.connect(self.log_view.append)
        self.ai_worker.progress_updated.connect(
            lambda i, t: (self.progress.setRange(0, t), self.progress.setValue(i))
        )
        self.ai_worker.analysis_complete.connect(
            lambda plan: self._on_analysis_complete(plan, dest)
        )
        self.ai_worker.error_occurred.connect(self._on_ai_error)
        self.cancel_action.setEnabled(True)
        self.ai_worker.start()

    def _on_analysis_complete(self, plan: list, dest: Path):
        """Stops progress and opens Preview Dialog."""
        self.progress.setVisible(False)
        self.set_controls_enabled(True)
        self.ai_worker = None

        if not plan:
            QMessageBox.information(self, "Done", "No items to organize.")
            self.lbl_status.setText(self.tr.t("ready"))
            return

        dialog = AIPreviewDialog(plan, dest, self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            final_plan = dialog.get_final_plan()
            self._execute_ai_plan(final_plan, dest)

    def _execute_ai_plan(self, plan: list, dest: Path):
        """Executes the AI organization plan after user approval."""
        self.set_controls_enabled(False)
        self.progress.setVisible(True)
        ...
        for i, item in enumerate(plan, 1):
            src = item["path"]
            dest_folder = dest / item["dest"]
            dest_path = dest_folder / src.name

            final_dest = file_organizer.resolve_conflict(dest_path, conflict)
            if final_dest is None:
                status = "Skipped"
            else:
                success = file_organizer.do_transfer(src, final_dest, action, dry_run)
                status = "Success" if success else "Failed"

            self.on_result_logged(
                str(src),
                str(final_dest) if status == "Success" else "N/A",
                src.name,
                status
            )
        ...
        succeeded = sum(1 for _ in range(self.table_view.rowCount())
                        if self.table_view.item(_, 2) and self.table_view.item(_, 2).text() == "Success")
        QMessageBox.information(
            self, self.tr.t("done"),
            f"AI Organization complete!\nTotal: {len(plan)}, Succeeded: {succeeded}"
        )

    def _on_ai_error(self, error_msg: str):
        """Handles AI worker errors."""
        self.progress.setVisible(False)
        self.set_controls_enabled(True)
        self.ai_worker = None
        self.lbl_status.setText(self.tr.t("ready"))
    ...
    def cancel_organizer(self):
        ...
        elif hasattr(self, 'ai_worker') and self.ai_worker and self.ai_worker.isRunning():
            reply = QMessageBox.question(
                self,
                self.tr.t("cancel"),
                "Are you sure you want to cancel the AI analysis?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            if reply == QMessageBox.StandardButton.Yes:
